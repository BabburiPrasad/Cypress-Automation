...
describe('Log-In Module', () => {
    ...
    //     .then((text) => {
    //         const expectedText = "Oops! It seems there's an issue:";
    //         expect(text.trim()).to.equal(expectedText);
    //     })
    // })

    it('TC_12 Verify Error message when user name is invalid', () =>{

        cy.get('.alert.alert-danger').should('include.text', 'Oops! It seems there\'s an issue:');
        cy.get('.alert.alert-danger').should('include.text', 'Check your email and password. Give it another shot!');
        cy.get('.alert.alert-danger').should('include.text', "Haven't verified your account? No problem! Look for a verification link in your email. If you can't find it, hit 'Forgot Password' for help.");
    })
    ...
    it('TC_13 Verify Login functionality with Valid Email Id and Invalid password', () => {
        ...
        login.clickonUserNametextbox('1t6xj3fzmj@happy2023year.com');
        cy.wait(2000);
